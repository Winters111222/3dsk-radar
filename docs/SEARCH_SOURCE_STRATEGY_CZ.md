...
**C — široký run — PŘIBLIŽNĚ 70 %.** Implementovaný je autentizovaný multi-source run state, immutable plán, cursory/chunky, průběžné oddělené ukládání kandidátů, retry/idempotency contract, cancel, cross-source/tender-revision dedupe, hard profily a cost reservation ledger. Offline acceptance nabízí 501 stran / 215 kandidátů a prokáže hard stop na 500 / 180; samostatné testy kryjí 403/429/timeout a přerušení bez redispatch. `paid_execution` je zamčené: Netlify Blobs contract nemá atomický compare-and-swap, takže před placenou cestou zbývá coordinator, detailní enrichment, Phase A promotion a UI progress. Podrobnosti: [Phase C run engine](PHASE_C_RUN_ENGINE_CZ.md).
