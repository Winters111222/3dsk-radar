...
**B — první funkční sběr — ROZPRACOVÁNO, PŘIBLIŽNĚ 50 %.** TED read adapter, samostatný autentizovaný endpoint, pevné query packs, 30denní ACTIVE filtr, caps/cooldown, parser, counters a offline mock acceptance jsou implementované za `RADAR_SOURCE_COLLECTION_ENABLED=false`. Dva malé anonymní produkční API canary requesty potvrdily HTTP 200, expert-query syntaxi a reálný response shape; relevantní pack měl v daném okamžiku 0 výsledků, takže výtěžnost zatím prokázaná není. Polycount/Unreal/Blender jsou explicitně `BLOCKED_ACCESS_REVIEW`, ne falešně vydávané za aktivní crawlery. Další stav a hranice popisuje [Phase B](PHASE_B_SOURCE_COLLECTION_CZ.md).

**C — široký run.** Přidat limitovaný běh/chunky, průběžné ukládání, retry/idempotence, cancel a cost reservations. V mock transportu nasimulovat 500 stránek, více než 12 platných výsledků, duplicity napříč zdroji, 403/429/timeout, prázdné JS stránky a přerušení po paid requestu. Prověřit zachování team state a rozpočtové omezení přes všechny pokusy.
...
|---|---|---|
| [Polycount — paid freelance](https://polycount.com/categories/freelance-job-postings) · P1 | HTML; `HTML_AFTER_REVIEW` | Placené freelance poptávky; první post a jeho editace, nikoli nabídky autorů v odpovědích. CLOSED/FILLED a USA-only vyřadit. |
| [Unreal — Job Offerings](https://forums.unrealengine.com/c/community/got-skills-looking-for-talent/job-offerings/76) · P1 | HTML; `HTML_OR_FEED_AFTER_REVIEW` | MetaHuman, rigging, character setup. I Job Offerings obsahuje LFW/sellery/revshare. Feed/JSON není provozně ověřen. |
| [Blender Artists — Paid Work](https://blenderartists.org/c/jobs/paid-work/53) · P1 | HTML; `HTML_OR_FEED_AFTER_REVIEW` | Placené 3D zakázky včetně charakterů a riggingu; odlišit drobné props od core služby. RSS/JSON neotestován. |
...
| [Find a Tender](https://www.find-tender.service.gov.uk/) · P1 | Dokumentace; `OCDS_API` | OCDS read data dokumentována GOV.UK; detailní API docs direct open 403. Provozní adapter neotestován. [Evidence 1](https://www.gov.uk/government/publications/open-contracting), [Evidence 2](https://www.find-tender.service.gov.uk/Developer/Documentation). |
| [Contracts Finder](https://www.contractsfinder.service.gov.uk/) · P1 | Dokumentace; `OCDS_API` | Použít OCDS read/download, nezaměnit publishing API. Překryv s FTS podle procurement ID. [Evidence 1](https://www.gov.uk/government/publications/open-contracting). |
...
| [CanadaBuys](https://canadabuys.canada.ca/en/tender-opportunities) · P1 | Dokumentace; `OPEN_DATA_OR_FEED` | Open/cancelled/expired jsou ve stejných datech. Feed/download endpoint vyřešit před enable; některé dokumenty SAP login. [Evidence 1](https://open.canada.ca/data/en/dataset/6abd20d4-7a1c-4b38-baa2-9525d0bb2fd2), [Evidence 2](https://canadabuys.canada.ca/en/support/searching-tender-opportunities). |
