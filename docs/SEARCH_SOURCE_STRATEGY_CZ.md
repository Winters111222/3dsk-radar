...
**B — první funkční sběr — ROZPRACOVÁNO, PŘIBLIŽNĚ 88 %.** TED, UK Find a Tender a Contracts Finder read adaptery, samostatný autentizovaný endpoint, pevné query packs, 30denní filtry, caps/cooldown/timeout, cursor boundary, parsers, counters a offline mock acceptance jsou implementované za `RADAR_SOURCE_COLLECTION_ENABLED=false`. Čtyři malé anonymní produkční API canary requesty potvrdily tři response contracts za `$0`; relevantní výtěžnost zatím prokázaná není. Access review Polycount/Unreal/Blender je dokončený a všechny tři zdroje zůstávají explicitně `BLOCKED_ACCESS_REVIEW`, ne falešně vydávané za aktivní crawlery. Další stav a hranice popisují [Phase B](PHASE_B_SOURCE_COLLECTION_CZ.md) a [access review](PHASE_B_ACCESS_REVIEW_CZ.md).
