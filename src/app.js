...
const STATUS_STORAGE_KEY = "3dsk-radar-stage1-status-v1";
const ACCESS_SESSION_KEY = "3dsk-radar-stage2-access-v1";
const state = {
  opportunities: [],
  selectedId: null,
  kind: "ALL",
...
function renderSummary() {
  ...
  const cards = [
    ["LAST SCAN", state.lastRun ? formatTimestamp(state.lastRun.completed_at) : "Fixture", state.lastRun ? `${state.lastRun.verified_source_count} verified web sources` : "$0 fixture mode"],
    ["NEW IN DATASET", all.filter((x) => x.is_new).length, state.datasetMode === "LIVE" ? "current live run" : "fixture records"],
    ["OPEN OPPORTUNITIES", open.length, `${all.length - open.length} potential lead${all.length - open.length === 1 ? "" : "s"}`],
    ["HIGH FIT", highFit.length, "FIT 80+"],
    ["CONTACTABLE", contactable.length, "public-source email only"]
  ];
  els.summary.innerHTML = cards.map(([label,value,sub]) => `<article class="summary-card"><span class="label">${escapeHtml(label)}</span><strong class="value">${escapeHtml(value)}</strong><span class="sub">${escapeHtml(sub)}</span></article>`).join("");
}

function renderTable() {
  const items = filtered();
  const sourceLabel = state.datasetMode === "LIVE" ? "live verified records" : "fixture records";
  els.count.textContent = `${items.length} shown · ${state.opportunities.length} total ${sourceLabel}`;
  if (!items.length) {
    els.body.innerHTML = `<tr><td colspan="10">No opportunities match this dataset and filter combination.</td></tr>`;
    return;
  }
...
function evidenceMarkup(item) {
  if (!item.source_evidence?.length) return "<p>No source evidence recorded.</p>";
  return `<ul class="evidence-list">${item.source_evidence.map((evidence) => `<li><a href="${escapeHtml(evidence.url)}" target="_blank" rel="noreferrer">${escapeHtml(evidence.type)}</a><span>${escapeHtml(evidence.note)}</span></li>`).join("")}</ul>`;
}
...
function renderDetail() {
  ...
  const contact = item.contact_email
    ? `<p><strong>${escapeHtml(item.contact_email)}</strong><br><span style="color:#748291">Provenance: <a href="${escapeHtml(item.contact_email_source)}" target="_blank" rel="noreferrer">public source</a></span></p>`
  ...
  els.detail.innerHTML = `
    ...
    <div class="detail-body">
      ...
      <div class="detail-section"><h4>SOURCE / FRESHNESS</h4><p>${escapeHtml(item.source_domain)} · ${escapeHtml(formatDate(item.published_date))} · first seen ${escapeHtml(formatTimestamp(item.first_seen))}</p></div>
      <div class="detail-section"><h4>STATUS</h4><p>${escapeHtml(item.status)} · Stage 2 still stores status only in this browser. Shared team history/dedupe arrives in Stage 3.</p></div>
...
function setStatus(id, status) {
  ...
  renderAll();
  showToast(`Status saved locally: ${status}`);
}
...
function searchRunNote(run) {
  const totalTokens = run?.usage?.total_tokens;
  const tokenText = Number.isFinite(totalTokens) ? ` · ${totalTokens.toLocaleString("en-US")} tokens` : "";
  const retryText = run?.attempts > 1 ? " · 1 structured retry used" : "";
  return `Live search complete · ${run.returned_count} verified records · ${run.verified_source_count} web sources${tokenText}${retryText}`;
}

async function runLiveSearch() {
  const accessCode = els.accessCode.value.trim();
  if (!accessCode) {
    showToast("Enter the team access code before running paid search.");
    els.accessCode.focus();
    return;
  }
  sessionStorage.setItem(ACCESS_SESSION_KEY, accessCode);

  const previousText = els.find.textContent;
  els.find.disabled = true;
  els.find.textContent = "SEARCHING CURRENT WEB…";
  els.scanNote.textContent = "Server-side search is running. Existing results stay visible if the run fails.";
  ...
  try {
    ...
    if (!response.ok || !payload.ok) {
      const retry = payload?.error?.retry_after_seconds ? ` Retry in ${payload.error.retry_after_seconds}s.` : "";
      throw new Error(`${payload?.error?.message || `Search failed (${response.status}).`}${retry}`);
    }

    state.opportunities = hydrateStatuses(Array.isArray(payload.opportunities) ? payload.opportunities : []);
    state.selectedId = state.opportunities[0]?.id || null;
    state.datasetMode = "LIVE";
    state.lastRun = payload.run || null;
    els.datasetPill.textContent = `Live search · ${state.opportunities.length} verified`;
    els.scanNote.textContent = searchRunNote(payload.run || {});
    renderAll();
    showToast(`Live Radar search returned ${state.opportunities.length} verified opportunities.`);
  } catch (error) {
    els.scanNote.textContent = `Search failed · previous dataset retained · ${error.message}`;
    showToast(error.message);
  } finally {
    els.find.disabled = false;
    els.find.textContent = previousText;
  }
}
...
els.body.addEventListener("click", (event) => {
  ...
  if (row) selectOpportunity(row.dataset.id);
});
...
document.querySelectorAll("[data-kind]").forEach((button) => button.addEventListener("click", () => {
  document.querySelectorAll("[data-kind]").forEach((x) => x.classList.remove("is-active"));
...
els.find.addEventListener("click", runLiveSearch);
