...
function directoryCompanies(){
  ...
  }).filter((entry)=>!query||[entry.company,...entry.contacts.flatMap((contact)=>[contact.label,contact.email]),entry.latestNote?.contact_note].filter(Boolean).join(" ").toLowerCase().includes(query)).sort((a,b)=>Number(unresolvedCompanyName(a.company))-Number(unresolvedCompanyName(b.company))||String(b.lastSeen||"").localeCompare(String(a.lastSeen||""))||a.company.localeCompare(b.company));
}
