...
function directoryCompanies(){
  ...
  }).filter((entry)=>!query||[entry.company,...entry.contacts.flatMap((contact)=>[contact.label,contact.email]),entry.latestNote?.contact_note].filter(Boolean).join(" ").toLowerCase().includes(query)).sort((a,b)=>String(b.lastSeen||"").localeCompare(String(a.lastSeen||""))||a.company.localeCompare(b.company));
}
function renderCompanyDirectory(){
  const companies=directoryCompanies();els.companyDirectoryCount.textContent=`${companies.length} companies`;
  if(!companies.length){els.companyDirectoryGrid.innerHTML=`<p class="archive-empty">${state.datasetMode==="DISCONNECTED"?"Enter your team access code to load the historical directory.":"No companies match this search."}</p>`;return;}
  els.companyDirectoryGrid.innerHTML=companies.map((entry)=>{const latestContact=entry.companyState.contact_history?.[0];return`<article class="archive-card company-directory-card"><div class="archive-card-head"><div><span class="archive-kicker">${entry.companyState.bookmarked?"★ SAVED COMPANY":"HISTORICAL COMPANY"}</span><h4>${escapeHtml(entry.company)}</h4></div><span class="outreach ${entry.companyState.last_contacted_at?"past":"none"}">${entry.companyState.last_contacted_at?"CONTACTED":"NEVER CONTACTED"}</span></div><div class="archive-metrics"><span><strong>${entry.items.length}</strong> opportunities</span><span>First ${escapeHtml(formatTimestamp(entry.firstSeen))}</span><span>Last ${escapeHtml(formatTimestamp(entry.lastSeen))}</span></div><div class="directory-people"><strong>PEOPLE / CONTACTS</strong>${entry.contacts.length?entry.contacts.slice(0,4).map((contact)=>`<span>${escapeHtml(contact.label)}${contact.email&&contact.email!==contact.label?` · ${escapeHtml(contact.email)}`:""}</span>`).join(""):"<span>No named public contact saved</span>"}</div><div class="directory-note"><strong>${latestContact?`${escapeHtml(CONTACT_CHANNEL_LABELS[latestContact.channel]||"Email")} · ${escapeHtml(formatTimestamp(latestContact.sent_at))}`:entry.latestNote?escapeHtml(CONTACT_CHANNEL_LABELS[entry.latestNote.contact_channel]||"Note"):"NO OUTREACH NOTE"}</strong><span>${escapeHtml(latestContact?.note||entry.latestNote?.contact_note||"Open the company to review its saved opportunities.")}</span></div><button class="action-button" data-open-company="${escapeHtml(entry.company)}" type="button">VIEW ${entry.items.length} OPPORTUNIT${entry.items.length===1?"Y":"IES"}</button></article>`;}).join("");
}
function renderSearchHistoryList(){
  ...
  els.searchHistoryGrid.innerHTML=runs.map((run)=>{const cost=Number.isFinite(Number(run.estimated_cost_usd))?`$${Number(run.estimated_cost_usd).toFixed(4)}`:"Cost unavailable";return`<article class="archive-card search-run-card"><div class="archive-card-head"><div><span class="archive-kicker">${escapeHtml(run.mode||"SEARCH")}</span><h4>${escapeHtml(formatTimestamp(run.completed_at))}</h4></div><span class="run-cost">${escapeHtml(cost)}</span></div><div class="search-run-metrics"><span><strong>${escapeHtml(run.new_opportunities||0)}</strong> new</span><span><strong>${escapeHtml(run.updated_opportunities||0)}</strong> updated</span><span><strong>${escapeHtml(run.returned_count||0)}</strong> returned</span><span><strong>${escapeHtml(run.workspace_total||0)}</strong> workspace</span></div><p>${escapeHtml(run.source_urls_verified||0)} verified source URLs · ${escapeHtml(run.web_search_call_count||0)} web searches · ${escapeHtml(run.openai_requests||0)} AI requests</p></article>`;}).join("");
}
...
els.companyDirectoryGrid.addEventListener("click",(event)=>{const button=event.target.closest("[data-open-company]");if(!button)return;const company=button.dataset.openCompany;savedSearch.value=company;state.query=company;state.view="ALL";document.querySelectorAll("[data-view]").forEach((item)=>item.classList.toggle("is-active",item.dataset.view==="ALL"));renderTable();document.querySelector(".results-layout").scrollIntoView({behavior:"smooth",block:"start"});});
...
document.querySelector("#signout-button").addEventListener("click",()=>{sessionStorage.removeItem(ACCESS_SESSION_KEY);els.accessCode.value="";state.opportunities=[];state.rejectedCandidates=[];state.searchHistory=[];state.companies=new Map();state.datasetMode="DISCONNECTED";state.selectedId=null;state.sourceRun=null;state.sourceCandidates=[];state.sourceRunStop=true;state.sourceRunMessage=null;state.ultraRun=null;state.ultraNextOperation=null;state.ultraStop=true;state.ultraMessage=null;els.datasetPill.textContent="Team access required";document.querySelector("#last-search").textContent="Connect to view saved search history";window.dispatchEvent(new CustomEvent("radar:search-history",{detail:null}));renderAll();});
