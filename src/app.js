...
function renderSearchDiagnostics(){
  ...
  if(!diagnostics||!Array.isArray(diagnostics.source_yield)){els.searchDiagnostics.hidden=true;els.sourceYieldGrid.innerHTML="";return;}
  ...
  els.sourceYieldGrid.innerHTML=diagnostics.source_yield.map((item)=>`<article class="run-counter"><span>${escapeHtml(item.source_label)}</span><strong>${escapeHtml(item.returned)} returned</strong><small>${escapeHtml(item.consulted_urls)} consulted URLs · ${escapeHtml(item.eligible_detail_urls)} detail URLs · ${escapeHtml(item.candidates_seen)} candidates · ${escapeHtml(item.candidates_rejected)} rejected</small></article>`).join("");
...
async function init(){
  ...
  try{const h=await(await fetch("/api/health")).json();state.collectionEnabled=h.source_collection==="ENABLED";state.searchEnabled=h.production_search==="READY";state.replyEnabled=h.production_reply==="READY";document.querySelector("#ai-state").textContent=state.searchEnabled?"Production search ready · one paid run per UTC day":h.paid_ai_state==="LOCKED"?"Live AI locked until final acceptance":"Live AI enabled · production search locked";els.find.disabled=!state.searchEnabled;els.find.textContent=state.searchEnabled?"FIND NEW OPPORTUNITIES":"FIND NEW OPPORTUNITIES · PAID LOCKED";renderSourceRun();renderDetail();}catch{document.querySelector("#ai-state").textContent="Server status unavailable";}
