...
const state = { opportunities:[], rejectedCandidates:[], searchHistory:[], companies:new Map(), selectedId:null, selectedCompanyKey:null, companySort:"RECENT", view:"ALL", status:"ALL", minFit:0, datasetMode:"DISCONNECTED", lastRun:null, categories:[], sortKey:"win_score", sortDirection:"desc", sourceRun:null, sourceCandidates:[], sourceRunBusy:false, sourceRunStop:false, sourceRunMessage:null, collectionEnabled:false, ultraRun:null, ultraNextOperation:null, ultraBusy:false, ultraStop:false, ultraMessage:null, ultraEnabled:false, ultraPaidEnabled:false, replyEnabled:false };
...
const els = {
  ...
  connect:document.querySelector("#connect-button"), scanNote:document.querySelector("#scan-note"), statusFilter:document.querySelector("#status-filter"), fitFilter:document.querySelector("#fit-filter"),
  ...
  resultFilters:document.querySelector("#result-filters"), resultsLayout:document.querySelector(".results-layout"), opportunityLanes:document.querySelector("#opportunity-lanes"), companyDirectory:document.querySelector("#company-directory"), companyDirectoryGrid:document.querySelector("#company-directory-grid"), companyDirectoryDetail:document.querySelector("#company-directory-detail"), companyDirectoryCount:document.querySelector("#company-directory-count"), companyDirectorySort:document.querySelector("#company-directory-sort"), searchHistoryPanel:document.querySelector("#search-history-list"), searchHistoryGrid:document.querySelector("#search-history-grid"), searchHistoryCount:document.querySelector("#search-history-count"), workspaceSearchLabel:document.querySelector("#workspace-search-label"),
...
const CONTACT_CHANNEL_LABELS={EMAIL:"Email",LINKEDIN:"LinkedIn",WEB_FORM:"Web form",OTHER:"Other"};
function contactChannelOptions(selected){return Object.entries(CONTACT_CHANNEL_LABELS).map(([value,label])=>`<option value="${value}" ${value===(selected||"EMAIL")?"selected":""}>${label}</option>`).join("");}
function contactNoteCell(item,sales){if(!sales)return'<span class="contact-note locked">LOCKED</span>';const channel=CONTACT_CHANNEL_LABELS[item.contact_channel]||null,note=String(item.contact_note||"").trim();return`<button class="contact-note ${channel||note?"has-note":""}" data-open-note="${escapeHtml(item.id)}" type="button"><strong>${escapeHtml(channel||"+ ADD NOTE")}</strong>${note?`<span>${escapeHtml(note)}</span>`:""}</button>`;}
function outreachMarkup(item){ const company=companyStateFor(item); if(!company.last_contacted_at)return'<span class="outreach none">NOT CONTACTED</span>'; const days=daysSince(company.last_contacted_at); const recent=days!==null&&days<=30; return`<span class="outreach ${recent?"recent":"past"}">CONTACTED ${days===0?"TODAY":`${days}D AGO`}</span><span class="company">${company.contact_count||1}× total</span>`; }

function renderSummary(){ const all=state.opportunities,sales=all.filter(isSalesOpportunityRecord),competitors=all.filter((item)=>recordKindOf(item)==="COMPETITOR"); const b2b=sales.filter((item)=>engagementTrack(item)==="B2B_STUDIO"),individual=sales.filter((item)=>engagementTrack(item)==="INDIVIDUAL_FREELANCE"); const bookmarked=new Set(sales.filter((x)=>x.company_bookmarked).map((x)=>companyMapKey(x.company))).size; const contacted=new Set(sales.filter((x)=>x.company_last_contacted_at).map((x)=>companyMapKey(x.company))).size; const cards=[["OPPORTUNITIES",sales.length,state.datasetMode.toLowerCase()],["B2B",b2b.length,"studio / vendor"],["INDIVIDUAL",individual.length,"freelance projects"],["REJECTED",state.rejectedCandidates.length,"manual review only"],["BOOKMARKED",bookmarked,"buyer companies"],["CONTACTED",contacted,"buyer companies"],["HIGH FIT",sales.filter((x)=>x.fit_score>=80).length,"sales FIT 80+"],["COMPETITORS",competitors.length,"intelligence only"]]; els.summary.innerHTML=cards.map(([l,v,s])=>`<article class="summary-card"><span class="label">${escapeHtml(l)}</span><strong class="value">${escapeHtml(v)}</strong><span class="sub">${escapeHtml(s)}</span></article>`).join(""); }
...
function renderUltraMax(){
  ...
  els.ultraMaxButton.textContent=state.ultraBusy?"SEARCHING AT ULTRA MAX…":active?"RESUME MAXIMUM SEARCH":state.ultraPaidEnabled?"FIND MAXIMUM OPPORTUNITIES · UP TO $15":"FIND MAXIMUM OPPORTUNITIES · NATIVE ONLY";
...
function renderRejectedTable(items){
  ...
  if(!items.length){els.body.innerHTML='<tr class="empty-row"><td colspan="16">No candidate-level rejection records are available. The 15 pre-truth rejections from the last production run were not persisted by the previous schema.</td></tr>';return;}
...
<td data-label="Source">${item.source_url?`<a class="source-link" href="${escapeHtml(item.source_url)}" target="_blank" rel="noreferrer">OPEN</a>`:'<span class="company">UNVERIFIED URL</span>'}</td><td data-label="Note"><span class="contact-note locked">LOCKED</span></td></tr>`).join("");
}
function unresolvedCompanyName(value){return /^(unnamed|anonymous|private\b)|name undisclosed/i.test(String(value||"").trim());}
function directoryCompanies(){
  const byCompany=new Map();
  for(const item of state.opportunities.filter(isSalesOpportunityRecord)){
    const key=companyMapKey(item.company);if(!byCompany.has(key))byCompany.set(key,{key,company:item.company,items:[]});byCompany.get(key).items.push(item);
  }
  const query=String(state.query||"").trim().toLowerCase();
  return [...byCompany.values()].map((entry)=>{
    const company=companyStateFor(entry.items[0]);
    const contacts=[];
    for(const item of entry.items){const label=[item.contact_name,item.contact_role].filter(Boolean).join(" · ")||item.contact_email;if(label&&!contacts.some((value)=>value.label===label&&value.email===item.contact_email))contacts.push({label,email:item.contact_email||null});}
    for(const contact of company.contact_history||[]){if(contact.recipient&&!contacts.some((value)=>value.email===contact.recipient))contacts.push({label:contact.recipient,email:contact.recipient});}
    const firstSeen=entry.items.map((item)=>item.first_seen).filter(Boolean).sort()[0]||null;
    const lastSeen=entry.items.map((item)=>item.last_seen).filter(Boolean).sort().at(-1)||null;
    const latestNote=[...entry.items].filter((item)=>item.contact_note).sort((a,b)=>String(b.contact_note_updated_at||"").localeCompare(String(a.contact_note_updated_at||"")))[0];
    return {...entry,companyState:company,contacts,firstSeen,lastSeen,latestNote};
  }).filter((entry)=>!query||[entry.company,...entry.contacts.flatMap((contact)=>[contact.label,contact.email]),entry.latestNote?.contact_note].filter(Boolean).join(" ").toLowerCase().includes(query)).sort((a,b)=>{const unresolved=Number(unresolvedCompanyName(a.company))-Number(unresolvedCompanyName(b.company));if(unresolved)return unresolved;const alphabetical=a.company.localeCompare(b.company,undefined,{sensitivity:"base",numeric:true});if(state.companySort==="A_Z")return alphabetical;if(state.companySort==="Z_A")return -alphabetical;return String(b.lastSeen||"").localeCompare(String(a.lastSeen||""))||alphabetical;});
}
function renderCompanyDirectory(){
  const companies=directoryCompanies(),contacted=companies.filter((entry)=>entry.companyState.last_contacted_at).length,withContacts=companies.filter((entry)=>entry.contacts.length).length,unresolved=companies.filter((entry)=>unresolvedCompanyName(entry.company)).length;
  els.companyDirectoryCount.textContent=`${companies.length} companies · ${withContacts} with contacts · ${contacted} contacted · ${unresolved} unnamed`;
  if(!companies.length){els.companyDirectoryGrid.innerHTML=`<p class="archive-empty">${state.datasetMode==="DISCONNECTED"?"Enter your team access code to load the historical directory.":"No companies match this search."}</p>`;els.companyDirectoryDetail.innerHTML="";return;}
  if(!companies.some((entry)=>entry.key===state.selectedCompanyKey))state.selectedCompanyKey=companies[0].key;
  els.companyDirectoryGrid.innerHTML=companies.map((entry)=>`<button class="company-directory-row ${entry.key===state.selectedCompanyKey?"is-selected":""}" data-select-company="${escapeHtml(entry.key)}" type="button"><span><strong>${escapeHtml(entry.company)}</strong><small>${entry.contacts[0]?escapeHtml(entry.contacts[0].label):unresolvedCompanyName(entry.company)?"Unresolved buyer":"No named public contact"}</small></span><span class="company-directory-row-meta"><strong>${entry.items.length}</strong> opportunit${entry.items.length===1?"y":"ies"}<small>${escapeHtml(formatTimestamp(entry.lastSeen))}</small></span></button>`).join("");
  const selected=companies.find((entry)=>entry.key===state.selectedCompanyKey),latestContact=selected.companyState.contact_history?.[0];
  els.companyDirectoryDetail.innerHTML=`<div class="company-detail-head"><div><span class="archive-kicker">${selected.companyState.bookmarked?"★ SAVED COMPANY":unresolvedCompanyName(selected.company)?"UNRESOLVED BUYER":"HISTORICAL COMPANY"}</span><h4>${escapeHtml(selected.company)}</h4><p>First found ${escapeHtml(formatTimestamp(selected.firstSeen))} · last found ${escapeHtml(formatTimestamp(selected.lastSeen))}</p></div><span class="outreach ${selected.companyState.last_contacted_at?"past":"none"}">${selected.companyState.last_contacted_at?"CONTACTED":"NEVER CONTACTED"}</span></div><section><div class="company-detail-label">PEOPLE &amp; CONTACTS</div>${selected.contacts.length?selected.contacts.map((contact)=>`<p><strong>${escapeHtml(contact.label)}</strong>${contact.email&&contact.email!==contact.label?`<br><span>${escapeHtml(contact.email)}</span>`:""}</p>`).join(""):'<p><strong>No named public contact</strong><br><span>Radar will never guess an email address.</span></p>'}</section><section><div class="company-detail-label">OUTREACH</div><p><strong>${latestContact?`${escapeHtml(CONTACT_CHANNEL_LABELS[latestContact.channel]||"Email")} · ${escapeHtml(formatTimestamp(latestContact.sent_at))}`:"No outreach recorded"}</strong><br><span>${escapeHtml(latestContact?.note||selected.latestNote?.contact_note||"Choose an opportunity to add a note or record contact.")}</span></p></section><section><div class="company-detail-label">SAVED OPPORTUNITIES · ${selected.items.length}</div>${[...selected.items].sort((a,b)=>String(b.last_seen||"").localeCompare(String(a.last_seen||""))).map((item)=>`<button class="company-opportunity-row" data-open-opportunity="${escapeHtml(item.id)}" type="button"><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.opportunity_kind==="OPEN_OPPORTUNITY"?"Open opportunity":"Potential lead")} · Fit ${escapeHtml(item.fit_score)} · ${escapeHtml(formatTimestamp(item.last_seen))}</small></span><span>OPEN</span></button>`).join("")}</section><button class="action-button company-view-all" data-open-company="${escapeHtml(selected.company)}" type="button">VIEW ALL ${selected.items.length} OPPORTUNIT${selected.items.length===1?"Y":"IES"}</button>`;
}
function renderSearchHistoryList(){
  const query=String(state.query||"").trim().toLowerCase();
  const runs=state.searchHistory.filter((run)=>!query||[run.mode,run.search_status,run.completed_at].filter(Boolean).join(" ").toLowerCase().includes(query));
  els.searchHistoryCount.textContent=`${runs.length} saved run${runs.length===1?"":"s"}`;
  if(!runs.length){els.searchHistoryGrid.innerHTML=`<p class="archive-empty">${state.datasetMode==="DISCONNECTED"?"Enter your team access code to load search history.":"No saved searches match this filter. Earlier searches still remain represented in Companies & people."}</p>`;return;}
  els.searchHistoryGrid.innerHTML=runs.map((run)=>{const rawCost=run.estimated_cost_usd,cost=rawCost!==null&&rawCost!==""&&Number.isFinite(Number(rawCost))?`$${Number(rawCost).toFixed(4)}`:"Cost unavailable";return`<article class="archive-card search-run-card"><div class="archive-card-head"><div><span class="archive-kicker">${escapeHtml(run.mode||"SEARCH")}</span><h4>${escapeHtml(formatTimestamp(run.completed_at))}</h4></div><span class="run-cost">${escapeHtml(cost)}</span></div><div class="search-run-metrics"><span><strong>${escapeHtml(run.new_opportunities||0)}</strong> new</span><span><strong>${escapeHtml(run.updated_opportunities||0)}</strong> updated</span><span><strong>${escapeHtml(run.returned_count||0)}</strong> returned</span><span><strong>${escapeHtml(run.workspace_total||0)}</strong> workspace</span></div><p>${escapeHtml(run.source_urls_verified||0)} verified source URLs · ${escapeHtml(run.web_search_call_count||0)} web searches · ${escapeHtml(run.openai_requests||0)} AI requests</p></article>`;}).join("");
}
function renderArchiveView(){
  const directory=state.view==="COMPANY_DIRECTORY",history=state.view==="SEARCH_HISTORY";
  els.companyDirectory.hidden=!directory;els.searchHistoryPanel.hidden=!history;els.resultFilters.hidden=directory||history;els.resultsLayout.hidden=directory||history;
  els.opportunityLanes.hidden=directory||history||state.view==="REJECTED";
  els.statusFilter.closest("label").hidden=directory||history;els.fitFilter.closest("label").hidden=directory||history;
  els.workspaceSearchLabel.textContent=directory?"Search companies or people":history?"Filter search history":"Search saved results";
  document.querySelector("#opportunity-search").placeholder=directory?"Company, person, email or note…":history?"Date, mode or status…":"Title, company or skill…";
  if(directory)renderCompanyDirectory();if(history)renderSearchHistoryList();
  return directory||history;
}
function renderTable(){ renderSort();if(renderArchiveView())return;const items=filtered(),salesTotal=state.opportunities.filter(isSalesOpportunityRecord).length,competitorTotal=state.opportunities.filter((item)=>recordKindOf(item)==="COMPETITOR").length; if(!items.some(x=>x.id===state.selectedId))state.selectedId=items[0]?.id||null; renderDetail(); if(state.view==="REJECTED"){renderRejectedTable(items);return;} document.querySelector("#results-title").textContent="Sales opportunities & competitor intelligence"; els.count.textContent=`${items.length} shown · ${salesTotal} sales · ${competitorTotal} competitors`; if(!items.length){els.body.innerHTML=`<tr class="empty-row"><td colspan="16">${state.datasetMode==="DISCONNECTED"?"Enter your team access code to load saved opportunities.":state.opportunities.length?"No records match these filters. Clear categories or adjust Status and Minimum fit.":"No saved opportunities yet. New searches will be saved here."}</td></tr>`;return;} els.body.innerHTML=items.map((item)=>{const budget=budgetView(item),sales=isSalesOpportunityRecord(item);return`<tr data-id="${escapeHtml(item.id)}" class="${item.id===state.selectedId?"is-selected":""}">
<td data-label="Select"><input class="select-radio" type="radio" name="selected-opportunity" aria-label="Select ${escapeHtml(item.title)}" ${item.id===state.selectedId?"checked":""}></td>
...
<td data-label="Source"><a class="source-link" href="${escapeHtml(item.source_url)}" target="_blank" rel="noreferrer">OPEN</a></td><td data-label="Note">${contactNoteCell(item,sales)}</td></tr>`;}).join(""); }
...
function historyMarkup(company){ if(!company.contact_history?.length)return"<p>Never contacted.</p>"; return`<ul class="history-list">${company.contact_history.slice(0,5).map((x)=>`<li><strong>${escapeHtml(formatTimestamp(x.sent_at))} · ${escapeHtml(CONTACT_CHANNEL_LABELS[x.channel]||"Email")}</strong><span>${escapeHtml(x.note||x.recipient||"Contact recorded")}${x.subject?` · ${escapeHtml(x.subject)}`:""}</span></li>`).join("")}</ul>`; }
...
<div class="detail-body"><div class="intelligence-lock"><strong>ALL SALES ACTIONS LOCKED</strong><span>This record describes a seller/service provider. It is excluded from Opportunities, buyer-company and High Fit totals. Contact, Generate Response and MARK CONTACTED remain disabled.</span></div>
...
<div class="detail-section contact-note-editor"><h4>NOTE / CONTACT CHANNEL</h4><label><span>CHANNEL</span><select data-contact-channel>${contactChannelOptions(item.contact_channel)}</select></label><label><span>NOTE</span><textarea data-contact-note maxlength="500" rows="3" placeholder="For example: Messaged the Art Director">${escapeHtml(item.contact_note||"")}</textarea></label><div class="contact-note-actions"><button class="action-button" data-save-contact-note="1" type="button">SAVE NOTE</button><button class="action-button sent" data-save-contacted="1" type="button" ${manualVerificationRequired(item)?"disabled":""}>SAVE &amp; MARK CONTACTED</button></div></div>
<div class="detail-section"><h4>COMPANY OUTREACH HISTORY</h4><p><strong>${company.contact_count||0} contact${company.contact_count===1?"":"s"} recorded</strong>${company.last_contacted_at?` · last ${escapeHtml(formatTimestamp(company.last_contacted_at))}`:""}</p>${historyMarkup(company)}</div>
<div class="detail-section"><h4>SOURCE EVIDENCE</h4>${evidenceMarkup(item)}</div>
${replyMarkup(item)}<div class="detail-actions">${manualVerificationRequired(item)?'<button class="action-button" type="button" disabled>CONTACT LOCKED</button>':item.contact_email?'<button class="action-button" data-copy="email" type="button">COPY EMAIL</button>':`<a class="action-button" href="${escapeHtml(item.apply_url)}" target="_blank" rel="noreferrer">OPEN CONTACT / APPLY</a>`}<a class="action-button" href="${escapeHtml(item.source_url)}" target="_blank" rel="noreferrer">${manualVerificationRequired(item)?"OPEN SOURCE · VERIFY":"OPEN SOURCE"}</a><button class="action-button primary full" data-generate-response="1" type="button" ${manualVerificationRequired(item)||replyLocked?"disabled":""}>${replyLocked?"GENERATE RESPONSE · PAID LOCKED":item.reply_body?"REGENERATE RESPONSE":"GENERATE RESPONSE"}</button><button class="action-button" data-copy-subject="1" type="button" ${item.reply_subject&&!manualVerificationRequired(item)?"":"disabled"}>COPY SUBJECT</button><button class="action-button" data-copy-response="1" type="button" ${item.reply_body&&!manualVerificationRequired(item)?"":"disabled"}>COPY RESPONSE</button></div></div>`; }
...
async function saveContactNote(markContacted=false){const item=state.opportunities.find((x)=>x.id===state.selectedId);if(!item)return;if(!isSalesOpportunityRecord(item)){showToast("Outreach is locked for competitor intelligence.");return;}if(markContacted&&manualVerificationRequired(item)){showToast("Verify the original source before recording outreach.");return;}const channel=els.detail.querySelector("[data-contact-channel]")?.value||"EMAIL",note=(els.detail.querySelector("[data-contact-note]")?.value||"").trim();if(state.datasetMode==="FIXTURE"){const now=new Date().toISOString();Object.assign(item,{contact_channel:channel,contact_note:note||null,contact_note_updated_at:now});if(markContacted){const current=companyStateFor(item),history=[{sent_at:now,channel,note:note||null,recipient:item.contact_email||null,subject:item.reply_subject||null,opportunity_id:item.id},...(current.contact_history||[])];applyCompanyState({...current,company:item.company,last_contacted_at:now,contact_count:history.length,contact_history:history});item.status="CONTACTED";saveFixtureStatus(item.id,"CONTACTED");}renderAll();showToast(markContacted?"Fixture contact recorded — preview only":"Fixture note saved — preview only");return;}if(!accessCode()){showToast("Enter team access code first.");return;}try{const payload=markContacted?await api("/api/company-state",{method:"POST",body:JSON.stringify({action:"MARK_CONTACTED",company:item.company,opportunity_id:item.id,channel,note})}):await api("/api/opportunity-status",{method:"POST",body:JSON.stringify({action:"SAVE_CONTACT_NOTE",opportunity_id:item.id,channel,note})});if(payload.company)applyCompanyState(payload.company);Object.assign(item,payload.opportunity||{});renderAll();showToast(markContacted?"Contact recorded · company history updated":"Contact note saved");}catch(error){renderDetail();showToast(error.message);}}
...
function applyTeamSnapshot(payload){
  state.opportunities=payload.records||payload.opportunities;state.rejectedCandidates=rejectedCandidatesFromSnapshot(payload);state.searchHistory=Array.isArray(payload.search_history)?payload.search_history:payload.last_search?[payload.last_search]:[];state.selectedId=(state.view==="REJECTED"?state.rejectedCandidates:state.opportunities)[0]?.id||null;state.companies=new Map();
...
els.body.addEventListener("click",(event)=>{const star=event.target.closest("[data-bookmark-company]");if(star){event.stopPropagation();toggleBookmark(star.dataset.bookmarkCompany);return;}const note=event.target.closest("[data-open-note]");if(note){event.stopPropagation();selectOpportunity(note.dataset.openNote);requestAnimationFrame(()=>els.detail.querySelector("[data-contact-channel]")?.focus());return;}if(event.target.closest("select,a,button,input"))return;const row=event.target.closest("tr[data-id]");if(row)selectOpportunity(row.dataset.id);});
...
els.detail.addEventListener("click",(event)=>{const star=event.target.closest("[data-bookmark-company]");if(star){toggleBookmark(star.dataset.bookmarkCompany);return;}const item=selectedRecord(),sales=item&&state.view!=="REJECTED"&&isSalesOpportunityRecord(item);if(event.target.dataset.rejectedDecision&&state.view==="REJECTED")setRejectedDecision(item.id,event.target.dataset.rejectedDecision);if(event.target.dataset.copy==="email"&&sales&&item.contact_email)copyText(item.contact_email,"Email");if(event.target.dataset.copySubject&&sales&&item.reply_subject)copyText(item.reply_subject,"Subject");if(event.target.dataset.copyResponse&&sales&&item.reply_body)copyText(item.reply_body,"Response");if(event.target.dataset.verifySource&&sales)verifySource();if(event.target.dataset.saveContactNote&&sales)saveContactNote(false);if(event.target.dataset.saveContacted&&sales)saveContactNote(true);if(event.target.dataset.generateResponse)generateResponse();});
function openCompanyOpportunities(company,id=null){savedSearch.value=company;state.query=company;state.view="ALL";state.selectedId=id||null;renderTable();document.querySelector(".results-layout").scrollIntoView({behavior:"smooth",block:"start"});}
els.companyDirectory.addEventListener("click",(event)=>{const select=event.target.closest("[data-select-company]");if(select){state.selectedCompanyKey=select.dataset.selectCompany;renderCompanyDirectory();return;}const opportunity=event.target.closest("[data-open-opportunity]");if(opportunity){const item=state.opportunities.find((entry)=>entry.id===opportunity.dataset.openOpportunity);if(item)openCompanyOpportunities(item.company,item.id);return;}const company=event.target.closest("[data-open-company]");if(company)openCompanyOpportunities(company.dataset.openCompany);});
els.companyDirectorySort.addEventListener("change",()=>{state.companySort=els.companyDirectorySort.value;renderCompanyDirectory();});
...
els.statusFilter.addEventListener("change",()=>{state.status=els.statusFilter.value;renderTable();}); els.fitFilter.addEventListener("change",()=>{state.minFit=Number(els.fitFilter.value);renderTable();}); els.connect.addEventListener("click",loadTeamState);els.sourceRunButton.addEventListener("click",runSourceCollection);els.sourceRunCancel.addEventListener("click",cancelSourceCollection);els.ultraMaxButton.addEventListener("click",runUltraMax);els.ultraMaxCancel.addEventListener("click",cancelUltraMax);
...
function renderSort(){
  const archiveView=state.view==="COMPANY_DIRECTORY"||state.view==="SEARCH_HISTORY";
  els.statusFilter.disabled=state.view==="REJECTED"||archiveView;
  els.fitFilter.disabled=state.view==="REJECTED"||archiveView;
  const opportunityViews=new Set(["ALL","B2B_STUDIO","INDIVIDUAL_FREELANCE","BOOKMARKED","OPEN_OPPORTUNITY","POTENTIAL_LEAD","COMPETITORS"]);
  document.querySelectorAll("[data-view]").forEach(button=>{const active=button.dataset.section==="opportunities"?opportunityViews.has(state.view):button.dataset.view===state.view;button.setAttribute("aria-pressed",String(active));button.classList.toggle("is-active",active);});
...
async function loadDemo(){
  ...
  state.opportunities=hydrateFixtureStatuses(await response.json());state.rejectedCandidates=[];state.searchHistory=[];state.companies=new Map();state.datasetMode="FIXTURE";state.lastRun=null;
...
document.querySelector("#signout-button").addEventListener("click",()=>{sessionStorage.removeItem(ACCESS_SESSION_KEY);els.accessCode.value="";state.opportunities=[];state.rejectedCandidates=[];state.searchHistory=[];state.companies=new Map();state.datasetMode="DISCONNECTED";state.selectedId=null;state.selectedCompanyKey=null;state.sourceRun=null;state.sourceCandidates=[];state.sourceRunStop=true;state.sourceRunMessage=null;state.ultraRun=null;state.ultraNextOperation=null;state.ultraStop=true;state.ultraMessage=null;els.datasetPill.textContent="Team access required";document.querySelector("#last-search").textContent="Connect to view saved search history";window.dispatchEvent(new CustomEvent("radar:search-history",{detail:null}));renderAll();});
...
async function init(){
  ...
  try{const h=await(await fetch("/api/health")).json();state.collectionEnabled=h.source_collection==="ENABLED";state.ultraEnabled=h.ultra_max_native==="READY";state.ultraPaidEnabled=h.ultra_max_paid==="READY";state.replyEnabled=h.production_reply==="READY";document.querySelector("#ai-state").textContent=state.ultraPaidEnabled?"ULTRA MAX search ready · $15 hard cap · no automatic retry":state.ultraEnabled?"ULTRA MAX native phase ready · paid phases locked":h.paid_ai_state==="LOCKED"?"Live AI locked until final acceptance":"Live AI enabled · ULTRA MAX locked";renderSourceRun();renderUltraMax();renderDetail();}catch{document.querySelector("#ai-state").textContent="Server status unavailable";}
