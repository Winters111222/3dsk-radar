...
function kindBadge(item){ if(recordKindOf(item)==="COMPETITOR")return'<span class="kind-badge competitor">COMPETITOR</span>'; const open=item.opportunity_kind==="OPEN_OPPORTUNITY"; return`<span class="kind-badge ${open?"open":"lead"}">${open?"OPEN OPPORTUNITY":"POTENTIAL LEAD"}</span>`; }
...
function renderSummary(){ const all=state.opportunities,sales=all.filter(isSalesOpportunityRecord),competitors=all.filter((item)=>recordKindOf(item)==="COMPETITOR"); const uniqueCompanies=new Set(sales.map((x)=>companyMapKey(x.company))).size; const bookmarked=new Set(sales.filter((x)=>x.company_bookmarked).map((x)=>companyMapKey(x.company))).size; const contacted=new Set(sales.filter((x)=>x.company_last_contacted_at).map((x)=>companyMapKey(x.company))).size; const cards=[["OPPORTUNITIES",sales.length,state.datasetMode.toLowerCase()],["BUYER COMPANIES",uniqueCompanies,"sales only"],["BOOKMARKED",bookmarked,"buyer companies"],["EMAILED",contacted,"buyer companies"],["HIGH FIT",sales.filter((x)=>x.fit_score>=80).length,"sales FIT 80+"],["COMPETITORS",competitors.length,"intelligence only"]]; els.summary.innerHTML=cards.map(([l,v,s])=>`<article class="summary-card"><span class="label">${escapeHtml(l)}</span><strong class="value">${escapeHtml(v)}</strong><span class="sub">${escapeHtml(s)}</span></article>`).join(""); }
...
function diagnosticReasonLabel(code){return({unverified_source_url:"source URL not verified",source_not_allowed_for_index_discovery:"URL was not an allowed opportunity detail",missing_core_identity:"missing title, company or summary",invalid_opportunity_kind:"invalid opportunity type",partner_without_buyer_signal:"partner label lacked a current subcontract/vendor signal",unknown_commercial_role:"buyer role not established",inactive_notice:"closed, awarded or cancelled",studio_ineligible:"studio/vendor not eligible",out_of_scope:"outside 3D.SK scope",stale_or_unverified:"older than 30 days without active proof",excluded_search_category:"excluded Visual / AI / Motion-only work",excluded_workflow:"excluded Reallusion / Character Creator / iClone / Daz3D workflow",individual_employment:"individual employee role without vendor route",heritage_capture_outside_cz_sk:"physical heritage capture outside CZ/SK",normalized_contract:"normalized data contract failed",other_validation_failure:"other validation failure"})[code]||code;}
