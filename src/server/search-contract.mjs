...
export function buildSearchInstructions({
  ...
  return [
    ...
    "Prioritize explicit current B2B/vendor/outsourcing/contract/freelance opportunities worldwide, especially the last 24h, then 7 days, then 30 days. You may inspect items up to 90 days old only when the exact original page still explicitly accepts proposals now.",
    "Apply gates in this exact order before assigning FIT or WIN: (1) exact source is active now, (2) the poster is a real buyer, (3) the buyer is purchasing a production deliverable rather than recruiting an employee, (4) a Czech/European studio or team is explicitly eligible, (5) the deliverable fits the approved scope and geography. If any gate fails, omit the item and do not score it.",
    ...
    "Do not return any employee vacancy, including permanent, full-time, fixed-term, entry-level, senior or lead roles, even when the employment page uses words such as contract, external development or B2B. A result must be a buyer project that can be delivered by an external studio/team and contain an explicit studio-vendor, outsourcing, subcontract, RFP/RFQ, paid-test, batch-production or production-overflow route.",
    ...
    "studio_eligibility is YES only when the exact brief positively supports delivery by a Czech/European external studio or vendor. UNKNOWN is not accepted as a sales opportunity. A country-only, onsite-only or individual-employment restriction is NO or UNKNOWN, never assumed YES.",
