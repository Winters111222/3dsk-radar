...
export const CONTACT_CHANNELS = Object.freeze(["EMAIL", "LINKEDIN", "WEB_FORM", "OTHER"]);
...
export function markContacted(current, { opportunityId, channel, note=null, recipient=null, subject=null, sourceUrl=null, sentAt }) { if (!sentAt) throw new TypeError("sentAt is required"); if (!CONTACT_CHANNELS.includes(channel)) throw new TypeError("valid channel is required"); const event={ id:randomUUID(), opportunity_id:opportunityId||null, sent_at:sentAt, channel, note:note||null, recipient:recipient||null, subject:subject||null, source_url:sourceUrl||null }; const history=[event,...(current.contact_history||[])].slice(0,100); return { ...current, last_contacted_at:sentAt, contact_count:history.length, contact_history:history, updated_at:sentAt }; }
export function markEmailSent(current, payload) { return markContacted(current, { ...payload, channel:"EMAIL" }); }
