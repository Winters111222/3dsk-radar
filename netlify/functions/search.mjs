...
function cooldownMs() { return boundedInt(envValue("RADAR_SEARCH_COOLDOWN_SECONDS"), 30, 0, 600) * 1000; }
...
export default async function handler(request, context) {
  ...
  if (runtime.inFlight || remaining > 0) return json({ ok:false, error:{ code:"SEARCH_RATE_LIMITED", message:runtime.inFlight ? "A search is already running." : "Search cooldown is active.", retry_after_seconds:runtime.inFlight ? 15 : Math.ceil(remaining / 1000) } }, 429);
  ...
  try {
    ...
    const result = await runOpportunitySearch({ apiKey, model, profile, nowIso, maxResults, allowStructuredRetry:true });
    ...
    return json({ ok:true, opportunities, run });
  } catch (error) {
