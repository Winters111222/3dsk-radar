...
test('director workspace preserves unique existing controls and hides technical sections initially',()=>{
  ...
  for(const id of ['ultra-max-button','source-run-button','connect-button','opportunity-body','detail-panel','opportunity-search','reset-filters'])assert.ok(ids.includes(id));
  assert.ok(!ids.includes('find-button'));
